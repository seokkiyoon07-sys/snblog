  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      <section aria-labelledby="search-heading">
        <h2 id="search-heading" className="sr-only">검색</h2>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <form onSubmit={handleSearch} className="space-y-4" role="search" aria-label="블로그 검색">
            <div className="flex gap-3">
              <div className="flex-1 relative">
                ...
                <div id="search-description" className="sr-only">
                  블로그 포스트를 검색할 수 있습니다. 제목, 내용, 태그로 검색 가능합니다.
                </div>
              </div>
              <button
                type="submit"
              ...
              >
                {isSearching ? '검색 중...' : '검색'}
                {isSearching && <span id="searching-status" className="sr-only">검색 중입니다. 잠시만 기다려주세요.</span>}
              </button>
            </div>
          </form>
        </div>
      ...
      {query && (
        <section aria-labelledby="results-heading">
          <h2 id="results-heading" className="sr-only">검색 결과</h2>
          <div className="space-y-4 lg:space-y-6">
            ...
            {searchResults.length > 0 ? (
              <div className="space-y-4 lg:space-y-6" role="list" aria-label="검색 결과 목록">
      ...
      {!query && (
        <section aria-labelledby="empty-state-heading">
          <h2 id="empty-state-heading" className="sr-only">검색 안내</h2>
