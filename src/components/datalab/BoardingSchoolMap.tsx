...
export default function BoardingSchoolMap() {
  ...
  // 테이블 정렬 상태
  const [sortColumn, setSortColumn] = useState<'name' | 'capacity' | 'price'>('price');
  ...
  // 지도에서 학원 위치로 이동
  const navigateToSchool = useCallback((school: BoardingSchool) => {
    const targetMap = isModalOpen ? modalMapInstanceRef.current : mapInstanceRef.current;
    if (targetMap && window.naver?.maps) {
      const latlng = new window.naver.maps.LatLng(school.lat, school.lng);
      targetMap.panTo(latlng);
      // 줌 레벨도 조정 (더 가까이)
      (targetMap as unknown as { setZoom: (zoom: number) => void }).setZoom(12);
  ...
  // 테이블 정렬 핸들러
  const handleSort = useCallback((column: 'name' | 'capacity' | 'price') => {
  ...
  return (
    <div className="space-y-4">
      ...
      {selectedSchool && !isModalOpen && !isDetailModalOpen && (
        <SchoolInfoCard school={selectedSchool} onClose={() => setSelectedSchool(null)} onNavigate={() => navigateToSchool(selectedSchool)} />
      )}
      ...
      <div className="mt-6 sm:mt-8 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-3 sm:px-4 py-2 sm:py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {filteredSchools.length}개 학원 | 클릭하면 지도에서 위치를 확인할 수 있어요
          </p>
        </div>
